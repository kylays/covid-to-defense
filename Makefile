...
# DEMOS = covid_to_defense bounce gravity pacman nbodies damping spaceinvaders wavy_circle pegs collision_check_nbodies collision_check_triangles breakout sound_demo text_demo image_demo
DEMOS = covid_to_defense
...
STUDENT_LIBS = vector list polygon color star body image text sound scene forces collision bullet tower virus global_body_info tool shop path
...
# LIBS = -lm -lSDL2 -lSDL2_gfx -lSDL_ttf
...
bin/wavy_circle: out/wavy_circle.o out/sdl_wrapper.o $(STUDENT_OBJS)
	$(CC) $(CFLAGS) $(LIBS) $^ -o $@
...
bin/collision_check_triangles: out/collision_check_triangles.o out/sdl_wrapper.o $(STUDENT_OBJS)
	$(CC) $(CFLAGS) $(LIBS) $^ -o $@

bin/collision_check_nbodies: out/collision_check_nbodies.o out/sdl_wrapper.o $(STUDENT_OBJS)
	$(CC) $(CFLAGS) $(LIBS) $^ -o $@

bin/covid_to_defense: out/covid_to_defense.o out/sdl_wrapper.o $(STUDENT_OBJS)
	$(CC) $(CFLAGS) $(LIBS) $^ -o $@

bin/sound_demo: out/sound_demo.o out/sdl_wrapper.o $(STUDENT_OBJS)
	$(CC) $(CFLAGS) $(LIBS) $^ -o $@

bin/text_demo: out/text_demo.o out/sdl_wrapper.o $(STUDENT_OBJS)
	$(CC) $(CFLAGS) $(LIBS) $^ -o $@

bin/image_demo: out/image_demo.o out/sdl_wrapper.o $(STUDENT_OBJS)
	$(CC) $(CFLAGS) $(LIBS) $^ -o $@
...
text:
	make bin/text_demo && ./bin/text_demo
...
bin/bounce.exe: out/bounce.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"

bin/gravity.exe: out/gravity.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"

bin/pacman.exe: out/pacman.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"
...
bin/collision_check_triangles.exe: out/collision_check_triangles.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"

bin/collision_check_nbodies.exe: out/collision_check_nbodies.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"

bin/wavy_circle.exe: out/wavy_circle.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"
...
bin/covid_to_defense.exe: out/covid_to_defense.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"

bin/sound_demo.exe: out/sound_demo.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"

bin/text_demo.exe: out/text_demo.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"

bin/image_demo.exe: out/image_demo.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"
# Builds the test suite executables from the corresponding test .o file
# and the library .o files. The only difference from the demo build command
# is that it doesn't link the SDL libraries.
bin/test_suite_%.exe bin\test_suite_%.exe: out/test_suite_%.obj out/test_util.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"
...
bin/wavy_circle bin\wavy_circle: bin/wavy_circle.exe;
bin/pegs bin\pegs: bin/pegs.exe;
bin/breakout bin\breakout: bin/breakout.exe;
bin/collision_check_triangles bin\collision_check_triangles: bin/collision_check_triangles.exe;
bin/collision_check_nbodies bin\collision_check_nbodies: bin/collision_check_nbodies.exe;
bin/covid_to_defense bin\covid_to_defense: bin/covid_to_defense.exe;
bin/sound_demo bin\sound_demo: bin/sound_demo.exe;
bin/text_demo bin\text_demo: bin/text_demo.exe;
bin/image_demo bin\image_demo: bin/image_demo.exe;
bin/test_suite_% bin\test_suite_%: bin/test_suite_%.exe ;

# CMD commands to test and clean

bin/student_tests.exe: out/student_tests.obj out/test_util.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) $(LIB_MATH) -link $(LINKEROPTS) $(LIBS) -out:"$
...
clean:
	for %%i in (out\* bin\*) \
	do (if not "%%~xi" == ".gitignore" (if not "%%~xi" == ".dll" (del %%~i)))
