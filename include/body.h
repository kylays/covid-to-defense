...
/**
 * @brief initializes a body with info and an image
 *
 * @param shape list of vectors describing body's initial shape
 * @param mass body mass
 * @param color body color on screen
 * @param info additional body info
 * @param info_freer if non-NULL, function call on info to free it
 * @param image_filename filename of image to display
 * @param image_size size of image to display
 * @return pointer to newly allocated body
 */
body_t *body_init_with_info_with_image(list_t *shape, double mass, rgb_color_t color, void *info,
                                       free_func_t info_freer, char *image_filename, int image_type, vector_t image_size);

/**
 * @brief initializes a body with text label and info
 *
 * @param shape list of vectors describing body's initial shape
 * @param mass body mass
 * @param body_color body color on screen
 * @param info additional body info
 * @param info_freer if non-NULL, function call on info to free it
 * @param text text to be displayed on top of body
 * @param text_color color of text to display
 * @param font font of text to display
...
/**
 * Releases the memory allocated for a body. Does NOT free the image or label.
 * Use image_free(body_get_image(body)) and text_free(body_get_label(body)).
 *
...
/**
 * Gets the size of a body.
 *
 * @param body a pointer to a body returned from body_init()
 * @return the max distance across the polygon shape
 */
...
/**
 * sets the information associated with a body.
 *
 * @param body a pointer to a body returned from body_init()
 * @param info a pointer to the new info
 */
...
/**
 * Sets the color of a body to a user specified value.
 *
 * @param body a pointer to a body returned from body_init()
 * @param color new color of body chosen by user
 */
void body_set_color(body_t *body, rgb_color_t color);

/**
 * @brief returns a pointer to the body's sprite image
 *
 * @param body body to get the image of
 * @return image associated with body
 */
image_t *body_get_image(body_t *body);

/**
 * @brief sets a body's sprite image
 *
 * @param body body to set the image of
 * @param image new body image
 */
void body_set_image(body_t *body, image_t *image);

/**
 * @brief returns a body's text label
 *
 * @param body body to get the label of
 * @return label associated with body
 */
text_t *body_get_label(body_t *body);

/**
 * @brief sets a body's label'
 *
 * @param body body to set the label of 
 * @param label new body label
 */
